...
public class DanMuPainter extends IDanMuPainter {
    ...
    protected void draw(Canvas canvas, DanMuModel danMuView, DanMuChannel danMuChannel) {
        ...
        if (!TextUtils.isEmpty(danMuView.name)) {
            drawTextName(danMuView, canvas, danMuChannel);
        }
    }

    protected void drawTextName(DanMuModel danMuView, Canvas canvas, DanMuChannel danMuChannel) {
        if (TextUtils.isEmpty(danMuView.name)) {
            return;
        }
        paint.setTextSize(danMuView.nameTextSize);
        paint.setColor(danMuView.nameTextColor);
        paint.setStyle(Paint.Style.FILL);
        CharSequence text = danMuView.name;
        ...
        float top = (int) (danMuView.getY() - 55)
                + danMuChannel.height / 2
                - staticLayout.getHeight() / 2;

        canvas.save();
        canvas.translate(((int) x-10), top);
    ...
    protected void drawTextBackground(DanMuModel danMuView, Canvas canvas, DanMuChannel danMuChannel) {
        ...
        Rect rectF = new Rect((int) x,
                ...
                (int) (top + textBackgroundHeight)+20);
