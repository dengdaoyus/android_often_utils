...
package top.zibin.luban.nvglunban;
...
public class NvgLuban {
    ...
    private static final String TAG = "NvgLuban";
    ...
    private NvgLuban(File cacheDir) {
        mBuilder = new LubanBuilder(cacheDir);
    }

    public static NvgLuban compress(Context context, File file) {
        NvgLuban nvgLuban = new NvgLuban(NvgLuban.getPhotoCacheDir(context));
        nvgLuban.mFile = file;
        nvgLuban.mFileList = Collections.singletonList(file);
        return nvgLuban;
    }

    public static NvgLuban compress(Context context, List<File> files) {
        NvgLuban nvgLuban = new NvgLuban(NvgLuban.getPhotoCacheDir(context));
        nvgLuban.mFileList = new ArrayList<>(files);
        nvgLuban.mFile = files.get(0);
        return nvgLuban;
    }
     ...
     */
    public static NvgLuban compress(File file, File cacheDir) {
        if (!isCacheDirValid(cacheDir)) {
            throw new IllegalArgumentException("The cacheDir must be Directory");
        }
        NvgLuban nvgLuban = new NvgLuban(cacheDir);
        nvgLuban.mFile = file;
        nvgLuban.mFileList = Collections.singletonList(file);
        return nvgLuban;
    }

    public static NvgLuban compress(List<File> files, File cacheDir) {
        if (!isCacheDirValid(cacheDir)) {
            throw new IllegalArgumentException("The cacheDir must be Directory");
        }
        NvgLuban nvgLuban = new NvgLuban(cacheDir);
     ...
     */
    public NvgLuban putGear(@GEAR int gear) {
        mBuilder.gear = gear;
        return this;
    }
     ...
     * 自定义图片压缩格式
     */
    public NvgLuban setCompressFormat(Bitmap.CompressFormat compressFormat) {
     ...
     */
    public NvgLuban setMaxSize(int size) {
     ...
     * @param width 最大宽度
     */
    public NvgLuban setMaxWidth(int width) {
     ...
     * @param height 最大高度
     */
    public NvgLuban setMaxHeight(int height) {
    ...
    private static File getPhotoCacheDir(Context context) {
        return getPhotoCacheDir(context, NvgLuban.DEFAULT_DISK_CACHE_DIR);
    }
    ...
    /**
     * 清空Luban所产生的缓存
     * Clears the cache generated by NvgLuban
     */
    public NvgLuban clearCache() {
